...
class EnsembleForecaster:
    ...
    def _predict_uncalibrated(self, player, fixture, history, dgw_idx=0, current_gw=1):
        ...
        if element_type == POS_DEF:
            # P(CBIT >= 10) * 2
            cbit = gpf_res.get("cbit90", 0.0) * cond_frac
            mu = cbit
            if mu > 0:
                v = 1.85 * mu
                p_nb = mu / v
                n = (mu**2) / (v - mu)
                xDefCon = p_play * max(0.0, 1.0 - nbinom.cdf(9, n, p_nb)) * 2.0
        elif element_type in [POS_MID, POS_FWD]:
            # P(CBIRT >= 12) * 2
            cbirt = gpf_res.get("cbirt90", 0.0) * cond_frac
            mu = cbirt
            if mu > 0:
                v = 1.85 * mu
                p_nb = mu / v
                n = (mu**2) / (v - mu)
                xDefCon = p_play * max(0.0, 1.0 - nbinom.cdf(11, n, p_nb)) * 2.0
        ...
        bps_defcon = gpf_res.get("cbirt90", 0.0) * min_frac * 0.33
